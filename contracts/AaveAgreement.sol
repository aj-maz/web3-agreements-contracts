...
contract AaveAgreement is IArbitrable {
    ...
    function start() public {
        ...
        require(
            aToken.balanceOf(address(this)) >= prize,
    ...
    function payEmployee() private {
        aaveLendingPool.withdraw(asset, prize, employee);
        aaveLendingPool.withdraw(asset, type(uint256).max, employee);
        employee.transfer(address(this).balance);
    }

    function reclaimPayment() private {
        aaveLendingPool.withdraw(asset, type(uint256).max, employer);
